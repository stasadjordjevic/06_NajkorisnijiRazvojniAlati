...
\begin{itemize}
  \item \textbf{Brzina} i \textbf{odziv okruženja} imaju najvišu prosečnu ocenu (4.74)
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  % traka pune sirine: od leve do desne ivice
  \fill[matfDarkRed]
    ([yshift=0.5*\bandh]current page.center -| current page.west) rectangle
    ([yshift=-0.5*\bandh]current page.center -| current page.east);

  % tekst tacno u centru
  \node[font=\bfseries\LARGE, text=white] at (current page.center) {Hvala na pažnji!};
\end{tikzpicture}
\end{frame}
