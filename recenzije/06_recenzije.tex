...
\title{Najkorisniji razvojni alati\\ \small{Staša Đorđević, Lazar Savić, Đurđa Milošević, Bogdan Tomić \\
...
\begin{enumerate}
\item Da li rad dobro odgovara na zadatu temu?\\
\odgovor{Da. Tema je obrađena u skladu sa zadatim ciljevima i istraživačkim pitanjima.}

\item Da li je nešto važno propušteno?\\
\odgovor{Ne. Dovoljno je obrađena tema kojom se rad bavi.}

\item Da li ima suštinskih grešaka i propusta?\\
\odgovor{Ne. Rad je sadržajno i metodološki korektan.}

\item Da li je naslov rada dobro izabran?\\
\odgovor{Da. Naslov je jasan, precizan i u skladu sa sadržajem rada.}

\item Da li sažetak sadrži prave podatke o radu?\\
\odgovor{Da. Sažetak jasno opisuje cilj, metodologiju i glavne nalaze.}

\item Da li je rad lak-težak za čitanje?\\
\odgovor{Rad je lak za čitanje i razumljiv ciljnoj grupi.}

\item Da li je za razumevanje teksta potrebno predznanje i u kolikoj meri?\\
\odgovor{Potrebno je osnovno predznanje iz oblasti informacionih tehnologija.}

\item Da li je u radu navedena odgovarajuća literatura?\\
\odgovor{Da. Literatura je relevantna i raznovrsna.}

\item Da li su u radu reference korektno navedene?\\
\odgovor{Da. Reference su pravilno i dosledno citirane.}

\item Da li je struktura rada adekvatna?\\
\odgovor{Da. Rad prati preporučenu strukturu seminarskog rada.}

\item Da li rad sadrži sve elemente propisane uslovom seminarskog rada (slike, tabele, broj strana...)?\\
\odgovor{Da. Rad ispunjava sve formalne zahteve.}

\item Da li su slike i tabele funkcionalne i adekvatne?\\
\odgovor{Da. Slike i tabele su pregledne i doprinose razumevanju rezultata.}
\end{enumerate}
